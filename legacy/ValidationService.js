...
const ValidationService = (function () {
  ...
  function _validateRole(value) {
    ...
    var allowed = ['SUPER_ADMIN', 'HOD', 'EVENT_ADMIN', 'COORDINATOR'];
    if (allowed.indexOf(target) !== -1) return null;
    
    // Auto-normalize fallback for Validation API usage
    var cleanTarget = target.replace(/[\s_]+/g, '');
    if (cleanTarget === 'SUPERADMIN' || cleanTarget === 'DEVELOPER') return null;
    if (cleanTarget === 'ADMIN' || cleanTarget === 'TESTER') return null;
    if (cleanTarget === 'EVENTADMIN' || cleanTarget === 'FACULTY' || cleanTarget === 'EVENTMANAGER') return null;
    if (cleanTarget === 'USER' || cleanTarget === 'STAFF' || cleanTarget === 'MONITOR' || cleanTarget === 'STUDENT' || cleanTarget === 'GUESTCOORDINATOR') return null;
