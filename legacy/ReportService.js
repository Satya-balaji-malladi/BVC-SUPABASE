...
const ReportService = {
  ...
  getCoordinatorReport: function (userId, targetCoordinatorId) {
    try {
      ...
      if (currentUser && currentUserRole === 'COORDINATOR') {
      ...
      let coordinators = allUsers.filter(u => (u['Role'] || u.role) === 'COORDINATOR');
  ...
  getCoordinatorPerformance: function (userId, coordinatorId) {
    try {
      ...
      let coordinators = cache.users.filter(u => (u['Role'] || u.role) === 'COORDINATOR');
