  ...
  Auth: {
    ...
    getUserAssignedEvents: function (sessionToken) {
      return SessionService.withSession(sessionToken, function(userId) {
        ...
        // SUPER_ADMIN and HOD can see events based on their own logic, but for assignments specifically:
        if (role === 'SUPER_ADMIN') {
          return allEvents;
        }
        ...
        var assignmentRoles = {}; // Track role per event for the UI

        assignments.forEach(function(a) {
          if (!a || a[CONFIG.COLUMNS.DELETION_FLAG]) return;
          var uId = String(a[CONFIG.COLUMNS.ASSIGNMENT_FACULTY_ID] || a['Faculty ID'] || a.faculty_id || a.user_id || '').trim();
          var eId = String(a[CONFIG.COLUMNS.ASSIGNMENT_EVENT_ID] || a['Event ID'] || a.event_id || '').trim();
          var aRole = String(a[CONFIG.COLUMNS.ASSIGNMENT_ROLE] || a['Assignment Role'] || '').trim().toUpperCase();
          if (uId === String(userId).trim() && eId) {
            assignedEventIds.add(eId);
            assignmentRoles[eId] = aRole;
          }
        });
        ...
        return allEvents.filter(function(e) {
          ...
          var eId = String(e[CONFIG.COLUMNS.EVENT_ID] || e.event_id || '').trim();
          e.assignmentRole = assignmentRoles[eId] || '';
          return e;
        });
      });
    },
    ...
    resolveEffectiveRole: function (sessionToken, eventId) {
      return SessionService.withSession(sessionToken, function(userId) {
        ...
        if (role === 'SUPER_ADMIN' || role === 'SUPER ADMIN') {
          return Utils.buildResponse(true, 'Success', { effectiveRole: 'SUPER_ADMIN', isGlobalAdmin: true });
        }
        ...
        var matches = assignments.filter(function(a) {
          var aEventId = String(a[CONFIG.COLUMNS.ASSIGNMENT_EVENT_ID] || a['Event ID'] || a.event_id || '').trim();
          var aFacultyId = String(a[CONFIG.COLUMNS.ASSIGNMENT_FACULTY_ID] || a['Faculty ID'] || a.faculty_id || a.user_id || '').trim();
          return aEventId === String(eventId).trim() && aFacultyId === String(userId).trim() && !a[CONFIG.COLUMNS.DELETION_FLAG];
        });
        
        if (matches.length > 0) {
          var match = matches[0];
          var effRole = String(match[CONFIG.COLUMNS.ASSIGNMENT_ROLE] || match['Assignment Role'] || match.role || match.Role || 'COORDINATOR').toUpperCase().replace(/[\s_]+/g, '');
          if (effRole === 'EVENTADMIN') effRole = 'EVENT_ADMIN';
